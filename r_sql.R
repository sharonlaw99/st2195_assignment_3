...
# codes to see what are in tables
head()
colnames()
...
print(q1[1,])   #show one row



q2 <- ontime_db %>%
  inner_join(carriers_db, by = c("Dest" = "iata")) %>%
  filter(Cancelled == 0) %>%
  group_by(city) %>%
  summarize(total = n()) %>%
  arrange(desc(total))

print(head(q2, 1))    #show one row
...
# part 4: A simplified solution for the query 4 in practice quiz
